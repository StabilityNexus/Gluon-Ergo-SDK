...
export class Gluon {
    ...
    async getFeeBoxes(gluonBox: GluonBox, ergVal: number, withOracleFee: boolean = true): Promise<{ devFee?: any, uiFee?: any, oracleFee?: any }> {
        ...
        if (this.config.ORACLE_FEE > 0 && withOracleFee) fees.oracleFee = await getOutBoxJs(this.config.ORACLE_FEE_TREE, Math.floor(Number(this.config.ORACLE_FEE * ergVal / 1e5)) + this.config.MIN_FEE)
        return fees
    }
